...
func TestIsSamePolicy(t *testing.T) {
	pubInst := "STARTING_MEADOW_ch1"
	privInst := "STARTING_MEADOW_acc1"
	pieInst := "studio_pie_acc1"

	if !isSamePolicy(pubInst, "STARTING_MEADOW", "acc1", false, false) {
		t.Fatal("expected public shard to match public policy")
	}
	if isSamePolicy(pubInst, "STARTING_MEADOW", "acc1", true, false) {
		t.Fatal("expected public shard NOT to match private policy")
	}
	if !isSamePolicy(privInst, "STARTING_MEADOW", "acc1", true, false) {
		t.Fatal("expected private shard to match private policy")
	}
	if !isSamePolicy(pieInst, "STARTING_MEADOW", "acc1", false, true) {
...
func TestHubLifecycleAndIdempotentJoin(t *testing.T) {
	...
	inst, err := wm.JoinMap("STARTING_MEADOW", accountID, false, false)
	if err != nil {
	...
	sameChar := reqMatches(p, "STARTING_MEADOW", charID, false, false)
	if !sameChar {
