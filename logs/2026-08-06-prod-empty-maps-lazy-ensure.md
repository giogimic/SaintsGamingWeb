...
PR **#43** merged and sockets are live (`/socket.io` Engine.IO handshake **200**).
`/api/maps` still returned `{"maps":[],"count":0}` → Studio/lobby `DEMO_SANDBOX` **404** loop.
Create New Map `POST /api/maps/TEST` → **500** (empty logic-tile catalog / failed upsert).
