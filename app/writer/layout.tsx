...
export default async function WriterLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!dbUser || (!dbUser.isWriter && dbUser.permissionLevel < PERMISSION_LEVELS.ADMIN)) {
    redirect("/not-found");
  }

  const WRITER_NAV = [
    { href: "/writer", label: "Dashboard", icon: PenTool },
    { href: "/writer/news", label: "My Articles", icon: Newspaper },
  ];
      ...
      <main className="flex-1 flex flex-col bg-background/95">
        <header className="h-16 border-b border-border/40 flex items-center px-6 md:hidden">
          ...
          <span className="ml-auto font-bold flex items-center gap-2">
            <PenTool className="h-4 w-4 text-emerald-400" /> Writer
          </span>
        </header>
