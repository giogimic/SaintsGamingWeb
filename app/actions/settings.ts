...
export async function getSiteVersion(isStatic = false): Promise<string> {
  if (isStatic) return process.env.NEXT_PUBLIC_SITE_VERSION || "2.1.879";

  try {
    const setting = await prisma.siteSetting.findUnique({ where: { key: 'SITE_VERSION' } });
    return setting?.value || process.env.NEXT_PUBLIC_SITE_VERSION || "2.1.879";
  } catch {
    // Fallback if DB not ready
    return process.env.NEXT_PUBLIC_SITE_VERSION || "2.1.879";
  }
}
