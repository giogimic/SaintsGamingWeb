...
export async function getSiteVersion(isStatic = false) {
  if (isStatic) return process.env.NEXT_PUBLIC_SITE_VERSION || "2.1.818";
  try {
    const setting = await prisma.siteSetting.findUnique({ where: { key: "SITE_VERSION" } });
    return setting?.value || process.env.NEXT_PUBLIC_SITE_VERSION || "2.1.818";
  } catch (error) {
    console.error("Failed to load SITE_VERSION, falling back:", error);
    return process.env.NEXT_PUBLIC_SITE_VERSION || "2.1.818";
  }
}
