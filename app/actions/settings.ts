...
export async function getSiteVersion(isStatic = false): Promise<string> {
  if (isStatic) return process.env.NEXT_PUBLIC_SITE_VERSION || "2.1.905";

  try {
    const setting = await prisma.siteSetting.findUnique({
      where: { key: 'SITE_VERSION' }
    });
    return setting?.value || process.env.NEXT_PUBLIC_SITE_VERSION || "2.1.905";
  } catch (error) {
    console.error("Failed to fetch site version:", error);
    return process.env.NEXT_PUBLIC_SITE_VERSION || "2.1.905";
  }
}
