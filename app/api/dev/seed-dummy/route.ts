...
export async function POST(_req: Request) {
  try {
    ...
    // Require Admin or Developer permission
    if (permissionLevel < PERMISSION_LEVELS.ADMIN && permissionLevel < PERMISSION_LEVELS.DEVELOPER) {
      return NextResponse.json({ message: "Unauthorized: Admin permissions required." }, { status: 401 });
    }

    // Resolve author user (prefer current session user, then any staff/admin user, then any existing user)
    let authorUser = null;
    if (session?.user?.id) {
      authorUser = await prisma.user.findUnique({ where: { id: session.user.id } });
    }
    if (!authorUser) {
    ...
    if (!authorUser) {
      return NextResponse.json({ 
        message: "No user found in database to assign as author. Please register an account first." 
      }, { status: 400 });
    }
    ...
    return NextResponse.json({ 
      success: true, 
      message: `Pushed ${createdCount} dummy articles and ${threadsCreated} forum threads successfully!` 
    });
  } catch (error: any) {
    console.error("[seed-dummy] Error:", error);
    return NextResponse.json({ 
      message: error?.message || "Internal Server Error seeding dummy content" 
    }, { status: 500 });
  }
}
