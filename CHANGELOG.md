...
- **Go MMO Migration V7:** Added `migrateV7` to automatically provision `WorldRelease` (with `id`, `projectId`, `version`, `manifestData`, `publishedBy`, `status`, `createdAt`) and `WorldMapSnapshot` tables in Go's SQLite database on startup.
...
- **Spawn Map Auto-Healing:** Added auto-healing in Go's `deployPublishedProjectRelease` and `ApplyReleaseMaps` so that if a canonical spawn map ID casing differs or is missing, it automatically resolves to the first available map in the release.
...
- Excluded system maps (DEMO_SANDBOX, STARTING_MAP) from Studio map validation and browsers.
- Centered Voxel and Tile browser panels.
- Resolved mapLogicTile creation error during database initialization.
...
- **Hyper-Modern Translucent Glass Feed & Responsive Bar Dimensions**:
  ...
  - **Desktop 3-Column Gaming Hub**: Added a dedicated Left Channel & Game Hub sidebar (`All Activity`, `Saints MMO`, `FiveM Highlights`, `Sandbox Modpacks`, `Clips & Reels`, `Hangout`) alongside the high-density center stream and community right sidebar.
...
- **2D-to-3D Item Rendering Subsystem (`ItemBillboardRenderer`)**: Created an optimized 3D billboard item rendering engine supporting upright Y-axis billboard alignment, Sandbox-style floating/bobbing animations, 3D spin rotation, and ground glow rings.
