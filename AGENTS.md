...
- `/studio` sets `isEditorMode`. Docs: `17`–`28` (through Studio backend). Prefer hot `content_reload` over process restart for data edits — see `26-studio-live-operations.md`. Production suite is `27`; backend contract (WorldMap SoT, studio services, no Prisma in client, no `io.emit` from routes) is `28-studio-backend-architecture.md`. Code/deploy still uses `scripts/update.sh` (hard-resets `origin/main`). Map reload today: `admin_reload_map` / `map_reloaded`. Canonical map save is `POST /api/maps/[slug]` (not socket `admin_save_map`).
