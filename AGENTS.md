...
- `DEMO_SANDBOX` needs `tileLayers` + `tilesets` with a **solid grass GID** (`DEFAULT_STUDIO_GROUND_GID = 17`). Filling with GID `1` paints stair fragments on black. `DemoBootstrap` / `ensureMapHasStudioTilesets` upgrade legacy all-GID-1 fills and sparse zeros. After pull, restart once. Studio skips title when already logged in (goes to character select). **Start Realm** on server-select is Admin+ only (hidden when logged out / non-admin).
