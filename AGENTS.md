...
- `/studio` sets `isEditorMode`. Docs: `17`–`26` (through live ops). Prefer hot `content_reload` over process restart for data edits — see `26-studio-live-operations.md`. Code/deploy still uses `scripts/update.sh` (hard-resets `origin/main`). Map reload today: `admin_reload_map` / `map_reloaded`.
