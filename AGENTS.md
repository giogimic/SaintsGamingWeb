...
- `/studio` sets `isEditorMode`. Docs: `17`–`33` (design complete through gap register). **Normative names:** `29-studio-glossary-canonical.md`. **Editor standard:** `30`. **Integrations:** `31`. Prefer hot `content_reload` over process restart — `26`/`28`. Production suite `27`. Backend `28`. Code/deploy: `scripts/update.sh` (hard-resets `origin/main`). Map reload today: `admin_reload_map` / `map_reloaded`. Canonical map save: `POST /api/maps/[slug]` (not socket `admin_save_map`). UI modes: Walk/Paint/Place/Populate/Script/Catalog (not Build/NPC/Test labels).
