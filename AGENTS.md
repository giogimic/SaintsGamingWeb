...
- Studio Asset Manager / Sprite Browser read `GameAsset` via `/api/assets` (not Prisma in the browser). If the table is empty, seed from disk: `npx tsx scripts/seed-game-assets-from-public.ts` (idempotent; scans `public/game-assets`).
