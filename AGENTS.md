...
- `/studio` sets `isEditorMode` (`src/shared/game/studioSession.ts`). Create tools (`isCreationMode`) soft-suppress encounters/combat; Walk Mode re-enables play-test. Phase 1 contracts: `info/gameplay-bible/17-studio-world-builder-economy.md`. Systems: `18-studio-master-architecture.md`. Full UX (tools, inspector, shortcuts, workflows): `19-studio-ux-design.md`. Loot Manager dock uses `/api/loot/tables`.
