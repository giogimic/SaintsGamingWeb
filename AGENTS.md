...
- `/studio` sets `isEditorMode`. Docs: `17`–`27` (through production tools). Prefer hot `content_reload` over process restart for data edits — see `26-studio-live-operations.md`. Production suite (projects/search/audit/team) is `27-studio-production-tools.md`. Code/deploy still uses `scripts/update.sh` (hard-resets `origin/main`). Map reload today: `admin_reload_map` / `map_reloaded`.
