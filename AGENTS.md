...
- `/studio` sets `isEditorMode` (`src/shared/game/studioSession.ts`). Create tools (`isCreationMode`) soft-suppress encounters/combat; Walk Mode re-enables play-test. Architecture: `info/gameplay-bible/17-studio-world-builder-economy.md`. Loot Manager dock uses `/api/loot/tables`.
- `DEMO_SANDBOX` must have non-empty `tileLayersData` + `tilesetsData` or Studio shows flat green with an empty Asset Picker. `DemoBootstrap` / `ensureMapHasStudioTilesets` backfill Ground + George tilesets when missing.
- **`DEMO_SANDBOX` was logic-grid-only** (`tileLayersData`/`tilesetsData` default `[]`). That renders as flat green colored ground (no TMX art) and leaves `TilesetPicker` empty so visual paint cannot start. `DemoBootstrap` now backfills a Ground layer + default George tilesets when empty; client also runs `ensureMapHasStudioTilesets` on load. After pull, restart the server (or set `FORCE_DEMO_MAP=1` once) so the DB row is patched. Logic (−1) paint still works without tilesets.
