...
export function InventoryWindow({ isOpen, onClose }: InventoryWindowProps) {
  const inventory = usePlayerStore((state) => state.player.inventory);
  const equipment = usePlayerStore((state) => state.player.equipment);
  const credits = usePlayerStore((state) => state.player.credits);
  const currency = usePlayerStore((state) => state.player.currency);
  const equipItem = usePlayerStore((state) => state.equipItem);
  const playerMaxWeight = usePlayerStore((state) => state.player.maxWeight);
  const playerPerk = usePlayerStore((state) => state.player.perk);
  ...
  const handleItemAction = (itemId: string, itemInfo: any) => {
    ...
    if (equipSlots.includes(typeUpper)) {
      ...
      if ((equipment as Record<string, string | null | undefined>)[slot] === itemId) {
        ...
        showToast(`Unequipped ${itemInfo.name}`);
      } else {
        equipItem(slot as any, itemId);
        showToast(`Equipped ${itemInfo.name}`);
      }
    } else if (typeUpper === 'FOOD' || typeUpper === 'CONSUMABLE') {
      if (itemInfo.stats?.hp) {
        socketManager.emit('use_item', { itemId });
  ...
  const handleDrop = (itemId: string, itemInfo: any) => {
    ...
    socketManager.emit('drop_item', { itemId });
    if (inventory[itemId] <= 1) setActiveItem(null);
  };
  ...
  return (
    ...
    >
      <div className="flex h-full w-full flex-col font-mono select-none">
