...
export interface GtcWindowProps {
  isOpen: boolean;
  onClose: () => void;
}

export function GtcWindow({ isOpen, onClose }: GtcWindowProps) {
  ...
  const handleBuyout = async (listing: TradeListing) => {
    ...
    socketManager.emit('gtc_purchase_listing', { listingId: listing.id });
  ...
  const handlePostListing = async () => {
    ...
    socketManager.emit('gtc_create_listing', {
      itemType: sellType,
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 h-full font-mono text-xs">
        ...
        <div className="flex flex-wrap justify-between items-center bg-black/50 p-2 rounded-xl border border-amber-500/30 gap-2 shrink-0">
        ...
        {activeTab === 'SELL' && (
          <div className="p-4 bg-black/40 border border-slate-800 rounded-xl space-y-4 flex-1 overflow-y-auto">
