...
import { calculatePlayerCombatStats } from '@/web/components/the-lobby/combat';
...
export function EquipmentWindow({ isOpen, onClose }: EquipmentWindowProps) {
  const equipment = usePlayerStore((state) => state.player.equipment) || {};
  const level = usePlayerStore((state) => state.player.level || 1);
  const activeDaemonId = usePlayerStore((state) => state.player.activeDaemonId);
  ...
  return (
    ...
    >
      <div className="flex h-full w-full flex-col font-mono text-xs select-none space-y-3">
