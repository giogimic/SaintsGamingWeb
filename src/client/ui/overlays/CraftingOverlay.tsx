...
import { Hammer, Sparkles, Shield, Sword, Wand2, FlaskConical, Filter, ChevronRight, Check, X } from 'lucide-react';
...
export function CraftingOverlay() {
  ...
  if (!openWindows.includes('crafting')) return null;
  ...
  const handleCraft = () => {
    ...
    const timer = setInterval(() => {
      ...
      if (currentStep >= steps) {
        ...
        socketManager.emit('craft_item', selectedRecipe.slug);
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md select-none pointer-events-none">
      ...
      >
        ...
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden font-mono p-4 gap-4 min-h-0">
          ...
          <div className="w-full md:w-7/12 flex flex-col justify-between bg-black/40 border border-cyan-500/20 rounded-2xl p-4 overflow-y-auto min-h-0">
            {selectedRecipe ? (
              <>
                <div>
                ...
                {/* Forging Action Bar */}
                <div className="pt-3 border-t border-cyan-500/20 mt-auto shrink-0">
                  {isCrafting ? (
