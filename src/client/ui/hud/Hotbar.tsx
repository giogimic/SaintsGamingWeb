...
import { getHudTheme } from '@/web/components/the-lobby/hud/hud-themes';
...
export function Hotbar() {
  const combatStyle = usePlayerStore((s) => s.player.combatStyle);
  const inventory = usePlayerStore((s) => s.player.inventory);
  const equippedAbilities = usePlayerStore((s) => s.player.equippedAbilities);
  
  const activeScene = useSessionStore((s) => s.activeScene);
  
  const combatTarget = useCombatStore((s) => s.combatTarget);
  const cooldowns = useCombatStore((s) => s.cooldowns);
  const setCooldown = useCombatStore((s) => s.setCooldown);

  const gameRegistry = useWorldStore((s) => s.gameRegistry);
  
  const hudThemeId = useHudStore((s) => s.hudThemeId);
  const hudConfig = useHudStore((s) => s.hudConfig);
  ...
  const abilities = useMemo((): HotbarAbility[] => {
    ...
    const registryAbilities = gameRegistry?.abilities || [];
  ...
  }, [equippedAbilities, gameRegistry]);
  ...
  const handleCast = (slot: (typeof slots)[number]) => {
    ...
    if (slot.action === 'ability') {
      if (isForbiddenRtCaptureAbility(slot.ability.id)) {
        showToast('Capture tools only work in creature battles.');
        return;
      }
      if (!combatTarget && slot.ability.type === 'damage') {
        soundSynth?.playSelectSound?.();
        showToast('Target required to cast offensive ability!');
        return;
      }
      ...
      // Send cast to server
      socketManager.emit('combat_cast', {
      ...
      if (potionKey) {
        ...
        socketManager.emit('use_item', { itemId: potionKey });
        setCooldown(slot.ability.id, timeNow + slot.ability.cooldownMs);
        setGlobalCooldown(timeNow + 1000);
      } else {
        soundSynth?.playSelectSound?.();
        showToast('No health potion or consumable in inventory!');
      }
    }
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const isCurrentlyPlayable = activeScene === 'exploring';
  ...
  }, [globalCooldown, combatTarget, slots, cooldowns, activeScene]);
  ...
  const isPlayable = activeScene === 'exploring';
  if (!isPlayable) {
    return null;
  }
