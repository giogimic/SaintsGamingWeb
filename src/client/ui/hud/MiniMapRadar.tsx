...
export function MiniMapRadar({ onOpenOptions, enableStudio = false }: MiniMapRadarProps) {
  ...
  const mapEntities = useWorldStore((state) => state.mapEntities);
  const activeMapData = useWorldStore((state) => state.activeMapData);
  
  const playerPos = usePlayerStore((state) => state.player.position);
  
  const otherPlayers = useMultiplayerStore((state) => state.otherPlayers);
  const partyMembers = useMultiplayerStore((state) => state.party);
  ...
  const draw = useCallback(() => {
    ...
    ctx.fill();
  }, [currentMapId, playerPos, mapEntities, otherPlayers, partyMembers, activeMapData]);
