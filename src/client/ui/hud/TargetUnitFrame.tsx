...
import { HeartContainersView } from '@/web/components/the-lobby/hud/HeartContainersView';
...
export function TargetUnitFrame() {
  const combatTarget = useCombatStore((state) => state.combatTarget);
  const setCombatTarget = useCombatStore((state) => state.setCombatTarget);
  const otherPlayers = useMultiplayerStore((state) => state.otherPlayers);
  const showToast = useToastStore((state) => state.showToast);

  const hudThemeId = useHudStore((state) => state.hudThemeId);
  const hudConfig = useHudStore((state) => state.hudConfig);
  ...
  const handlePartyInvite = () => {
    ...
    socketManager.emit('party_invite_send', { targetName: target.name });
  ...
  const handleDuelChallenge = () => {
    if (!target.name) return;
    soundSynth?.playActionSound?.();
    socketManager.emit('battle_invite_send', { targetId: target.entityId, targetName: target.name });
  ...
  const handleWhisper = () => {
    ...
    // In v2 we can use useChatStore, but for now we just emit the custom event or handle it in chat input.
    // For now we will keep the CustomEvent since it triggers the chat input focus.
