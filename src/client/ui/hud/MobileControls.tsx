...
import { useHudStore, type MobileControlMode } from '../../state/useHudStore';
...
function ActionCluster({
  ...
  const toggleInventory = () => {
    ...
    useHudStore.getState().toggleWindow('inventory');
  };

  const toggleSkills = () => {
    soundSynth?.playSelectSound?.();
    useHudStore.getState().toggleWindow('skills');
  };
...
export function MobileControls({
  ...
  const mode = useHudStore((s) => s.mobileControlMode);
  ...
  useEffect(() => {
    ...
    useHudStore.getState().hydrateMobileControlMode();
