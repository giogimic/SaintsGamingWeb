...
export function GameInitializationWizard({ isReinit = false }: { isReinit?: boolean }) {
  ...
  const [startingMap, setStartingMap] = useState<SetupStartingMapData>(() => ({
    ...
    spawnPoint: { x: 64, y: 16, z: 64 },
    gates: [{
      id: 'spawn', name: 'Genesis Gate', category: 'SPAWN',
      position: { x: 64, y: 16, z: 64 }, interactPrompt: 'Respawn',
    }],
  }));
  ...
  const handleCompleteSuccess = (_defaultMapId: string) => {
    router.push('/lobby');
  };
      ...
      <div className="min-h-[60vh] flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-[#050b14]/95 border border-red-500/40 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-xl">
          <div className="flex items-center justify-between px-4 py-2.5 bg-gradient-to-r from-red-950/40 via-[#0a1225] to-[#050b14] border-b border-red-500/30">
            <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-red-500/80" /><span className="font-mono text-xs font-semibold text-red-300 uppercase tracking-wider">Access Restricted</span></div>
            <button onClick={() => router.push('/')} className="text-slate-500 hover:text-white transition"><X className="w-3.5 h-3.5" /></button>
          </div>
          <div className="p-6 text-center space-y-4">
            <ShieldAlert className="w-10 h-10 text-red-400 mx-auto" />
            <div><h2 className="text-sm font-bold text-white uppercase tracking-wider">Administrator Access Required</h2><p className="text-xs text-slate-400 mt-1">Game Setup is restricted to server administrators and game developers.</p></div>
            <button onClick={() => router.push('/')} className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-xs font-mono font-semibold transition cursor-pointer">Return to Home</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto px-3 py-6 md:py-8 font-sans">
      <div className="pointer-events-auto select-none rounded-2xl border border-border/50 bg-[#050b14]/95 backdrop-blur-xl shadow-[0_12px_45px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2 border-b border-border/50 bg-slate-900/50 select-none">
          <div className="flex items-center gap-2 min-w-0"><Boxes className="w-4 h-4 text-amber-400 shrink-0" /><h2 className="text-xs font-black uppercase tracking-wider text-slate-100 truncate">Saints Game Studio — Onboarding</h2></div>
          <div className="flex items-center gap-1.5 shrink-0"><span className="text-[10px] font-mono px-2 py-0.5 rounded bg-amber-500/10 text-amber-500 border border-amber-500/20 font-semibold mr-1">v2.2.003</span><button type="button" onClick={() => router.push('/')} className="p-1 rounded text-slate-400 hover:text-rose-400 hover:bg-white/10 transition-all cursor-pointer" title="Close Wizard"><X className="w-4 h-4" /></button></div>
        </div>
        <div className="px-3 py-2 bg-[#08101e]/80 border-b border-border/40 overflow-x-auto flex items-center justify-between gap-1 text-xs font-mono select-none">
          <div className="flex items-center gap-1 min-w-max">{STEP_LABELS.map(({ num, label }) => { const isCurrent = step === num; const isPast = step > num; return <button key={num} type="button" onClick={() => { if (num < step) setStep(num); }} disabled={num > step} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs transition-all ${isCurrent ? 'bg-amber-500/20 text-amber-300 border border-amber-500/50 font-bold shadow-sm' : isPast ? 'text-emerald-400 hover:bg-emerald-500/10 hover:text-emerald-300 cursor-pointer' : 'text-slate-600 opacity-60 cursor-not-allowed'}`}><span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold ${isCurrent ? 'bg-amber-500 text-slate-950 font-black' : isPast ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-800 text-slate-500'}`}>{isPast ? <CheckCircle2 className="w-3 h-3" /> : num + 1}</span><span>{label}</span></button>; })}</div>
          <div className="hidden md:flex items-center gap-2 text-[11px] text-slate-400 font-mono shrink-0 pl-2"><span>Step {step + 1} of {STEP_LABELS.length}</span></div>
        </div>
        {errorMessage && <div className="mx-4 mt-4 p-3 rounded-xl bg-rose-950/40 border border-rose-500/40 text-rose-300 text-xs flex items-center gap-2.5"><ShieldAlert className="w-4 h-4 shrink-0 text-rose-400" /><span>{errorMessage}</span></div>}
        {setupStatus && !setupStatus.isFreshInstall && <div className="mx-4 mt-4 p-3.5 rounded-xl bg-[#081224] border border-amber-500/30 flex items-center justify-between gap-3 text-xs"><div className="flex items-center gap-2 min-w-0"><Info className="w-4 h-4 text-amber-400 shrink-0" /><span className="text-slate-300 truncate">Existing realm detected (<strong className="text-amber-300">{setupStatus.mapCount} maps</strong>). Setup preserves live database state.</span></div><div className="flex items-center gap-2 shrink-0"><button onClick={() => router.push('/studio')} className="px-3 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 font-mono text-[11px] font-semibold transition cursor-pointer">Studio</button><button onClick={() => router.push('/lobby')} className="px-3 py-1 rounded-lg bg-amber-600 hover:bg-amber-500 text-white font-mono text-[11px] font-bold transition cursor-pointer">Play Lobby</button></div></div>}
