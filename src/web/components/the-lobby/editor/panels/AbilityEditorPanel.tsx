...
import { ProfessionEditorPanel } from "./ProfessionEditorPanel";
import { StatusEditorPanel } from "./StatusEditorPanel";
import { CombatSimulatorPanel } from "./CombatSimulatorPanel";
...
export function AbilityEditorPanel() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#050b14] text-slate-200 text-sm overflow-hidden border border-border/50 rounded-lg">
      
      {/* Top Navigation Tabs */}
      <div className="flex items-center bg-card/85 border-b border-border/50 px-4 py-2 gap-2">
        <span className="font-bold sg-text-gradient mr-4 text-base">Gameplay & Systems Studio</span>
        
        <button 
          onClick={() => setActiveTab("abilities")}
          className={`px-4 py-1.5 rounded-md transition-colors ${activeTab === "abilities" ? "bg-primary/20 text-primary border border-primary/40 font-medium" : "text-muted-foreground hover:bg-white/5"}`}
        >
          Abilities
        </button>
        <button 
          onClick={() => setActiveTab("professions")}
          className={`px-4 py-1.5 rounded-md transition-colors ${activeTab === "professions" ? "bg-primary/20 text-primary border border-primary/40 font-medium" : "text-muted-foreground hover:bg-white/5"}`}
        >
          Professions & Skills
        </button>
        <button 
          onClick={() => setActiveTab("status")}
          className={`px-4 py-1.5 rounded-md transition-colors ${activeTab === "status" ? "bg-primary/20 text-primary border border-primary/40 font-medium" : "text-muted-foreground hover:bg-white/5"}`}
        >
          Status Effects
        </button>
        <button 
          onClick={() => setActiveTab("combat")}
          className={`px-4 py-1.5 rounded-md transition-colors ${activeTab === "combat" ? "bg-primary/20 text-primary border border-primary/40 font-medium" : "text-muted-foreground hover:bg-white/5"}`}
        >
          Combat Simulator
        </button>
      </div>
