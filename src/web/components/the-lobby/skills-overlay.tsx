...
interface SkillsOverlayProps {
  /** Callback when a skill is selected for inspection */
  onSelectSkill?: (slug: string) => void;
  /** Callback to deselect the current skill */
  onDeselectSkill?: () => void;
}

export default function SkillsOverlay({ onSelectSkill, onDeselectSkill }: SkillsOverlayProps = {}) {
  const skills = useGameStore((state) => state.player.skills);
  const [selectedSkill, setSelectedSkillLocal] = React.useState<string | null>(null);

  const handleSkillClick = (slug: string) => {
    soundSynth?.playSelectSound?.();
    const isSelected = selectedSkill === slug;
    const newSlug = isSelected ? null : slug;
    setSelectedSkillLocal(newSlug);
    
    if (newSlug && onSelectSkill) {
      onSelectSkill(newSlug);
    } else if (!newSlug && onDeselectSkill) {
      onDeselectSkill();
    }
  };
  ...
  return (
    <div className="flex flex-col gap-3 p-3 font-mono text-xs select-none">
