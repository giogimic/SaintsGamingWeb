...
describe('Superior Slayer Monster Spawns & Relic Drops Engine', () => {
  it('prevents superior spawns if Bigger and Badder is locked or off-task', () => {
    // Locked unlock
    expect(rollSuperiorSpawn('stone_golem', false, true, 0.001).shouldSpawn).toBe(false);
    // Off-task
    expect(rollSuperiorSpawn('stone_golem', true, false, 0.001).shouldSpawn).toBe(false);
    // Invalid monster
  ...
  it('spawns superior variant when unlocked, on-task, and roll < 1/200', () => {
    const res = rollSuperiorSpawn('stone_golem', true, true, 0.002); // 0.002 < 1/200 (0.005)
    expect(res.shouldSpawn).toBe(true);
    expect(res.superior).toBeDefined();
    expect(res.superior?.superiorId).toBe('marble_stone_golem');
    expect(res.superior?.name).toBe('Marble Stone Golem');
  ...
  it('rolls superior unique relic loot drops accurately', () => {
    ...
    const heartDrop = rollSuperiorUniqueLoot('marble_stone_golem', 75, 0.001);
    ...
    const staffDrop = rollSuperiorUniqueLoot('marble_stone_golem', 75, 0.025);
