...
import {
  initializeWorldTreeGuardianState,
  applyDamageToWorldTreeGuardian,
  processPhase4BlightBleed,
} from './solakPhaseEngine';

describe('WorldTreeGuardian: The Grove Guardian Phase & Core Engine', () => {
  it('manages limb destruction to expose Blight Core in Phase 1', () => {
    const solak = initializeWorldTreeGuardianState(1); // 3,500,000 HP
    expect(solak.phase).toBe(1);
    expect(solak.isCoreExposed).toBe(false);

    // Destroy Left Leg
    applyDamageToWorldTreeGuardian(solak, 'LEFT_LEG', solak.leftLeg.maxHp);
    expect(solak.leftLeg.hp).toBe(0);
    expect(solak.isCoreExposed).toBe(false);

    // Destroy Right Leg -> Core Exposed
    applyDamageToWorldTreeGuardian(solak, 'RIGHT_LEG', solak.rightLeg.maxHp);
    expect(solak.rightLeg.hp).toBe(0);
    expect(solak.isCoreExposed).toBe(true);

    // Attack Core -> Damage applies directly to WorldTreeGuardian
    const coreHit = applyDamageToWorldTreeGuardian(solak, 'CORE', 200000);
  ...
  it('progresses through Phase 2, Phase 3 Mind Realm, and Phase 4 DPS race', () => {
    const solak = initializeWorldTreeGuardianState(1);

    // Advance to Phase 2 (<= 75% = 2.625M HP)
    applyDamageToWorldTreeGuardian(solak, 'MAIN_BODY', 900000);
    ...
    applyDamageToWorldTreeGuardian(solak, 'MIND_MANIFESTATION', 500000);
