...
export function resolveSlopeShape(
  ...
  if (isVoxelSolid(above.high)) {
    return { shapeId: VoxelShape.FULL_CUBE, orientation: VoxelOrientation.NORTH };
  }

  // Check 4 cardinal neighbors at same height Y
  const northSolid = isVoxelSolid(world.getVoxel(wx, wy, wz + 1).high);
  const southSolid = isVoxelSolid(world.getVoxel(wx, wy, wz - 1).high);
  const eastSolid = isVoxelSolid(world.getVoxel(wx + 1, wy, wz).high);
  const westSolid = isVoxelSolid(world.getVoxel(wx - 1, wy, wz).high);

  // Check 4 cardinal neighbors at Y - 1 (below)
  const northBelowAir = isVoxelAir(world.getVoxel(wx, wy - 1, wz + 1).low);
  const southBelowAir = isVoxelAir(world.getVoxel(wx, wy - 1, wz - 1).low);
  const eastBelowAir = isVoxelAir(world.getVoxel(wx + 1, wy - 1, wz).low);
  const westBelowAir = isVoxelAir(world.getVoxel(wx - 1, wy - 1, wz).low);
...
export function resolveTerrainStratigraphy(
  ...
  for (let y = wy + 1; y < world.totalHeightBlocks; y++) {
    if (isVoxelSolid(world.getVoxel(wx, y, wz).high)) {
      solidAboveCount++;
    } else {
      break;
    }
