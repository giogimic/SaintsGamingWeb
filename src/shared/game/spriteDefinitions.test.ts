...
describe('Sprite Definitions & Animation Profiles', () => {
  it('resolves explicit classic_3x4 animation profile', () => {
    const def = resolveSpriteDefinition({ animationProfile: 'classic_3x4' });
    expect(def.profile).toBe('classic_3x4');
  ...
  it('resolves explicit modular-full animation profile', () => {
    ...
    expect(def.isModular).toBe(true);
  ...
  it('resolves explicit modular-walk animation profile', () => {
  ...
  it('infers modular-full from dimensions 832x1344 when profile is absent', () => {
  ...
  it('infers modular-walk from dimensions 576x256 when profile is absent', () => {
  ...
  it('infers classic_3x4 from dimensions 96x128 when profile is absent', () => {
  ...
  it('infers modular-full from extended height 832x3456 when profile is absent', () => {
  ...
  it('infers modular-full from item- prefix in URL', () => {
  ...
  it('converts definition to Babylon SpriteSheetConfig', () => {
    const modularConfig = spriteDefinitionToBabylonConfig(MULTI_FRAME_DIRECTIONAL_PROFILE);
    expect(modularConfig.columns).toBe(13);
    expect(modularConfig.rows).toBe(21);
    expect(modularConfig.directions.up).toBe(8);
    expect(modularConfig.walkCycle).toHaveLength(9);

    const classicConfig = spriteDefinitionToBabylonConfig(LEGACY_3X4_PROFILE);
    expect(classicConfig.columns).toBe(3);
    expect(classicConfig.rows).toBe(4);
    expect(classicConfig.directions.down).toBe(0);
    expect(classicConfig.walkCycle).toEqual([0, 1, 2, 1]);
  });
});
