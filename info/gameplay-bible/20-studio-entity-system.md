...
**This document is the entity master.** Existing `entitySchemas.ts`, `lootRefs.ts`, `mapLayers.ts`, `logicComponents.ts`, `WorldMap.npcsData` / `gatesData` / Logic grid are **inputs to unify** — not systems to rewrite in one shot.
