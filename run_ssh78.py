...
async function run() {
  ...
  const res = await fetch('http://saints-lobby:24011/api/internal/deploy-release', {
    ...
    headers: {
      ...
      'Authorization': 'Bearer 0xSaintsGamingInternalAdmin' // Wait, I need the right internal key. Let me just use standard fetch since there's no auth? Wait, there IS auth on deploy-release.
    },
  ...
  // also update game character default map
...
try:
    ...
    cmd = f"echo '{b64_js}' | base64 -d > ~/sync.js && sudo docker cp ~/sync.js saints-gaming-web:/app/sync.js && sudo docker exec saints-gaming-web npx tsx sync.js"
