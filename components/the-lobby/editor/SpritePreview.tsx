...
export const SpritePreview: React.FC<SpritePreviewProps> = ({ asset, onClose, onSelect }) => {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 space-y-4 shadow-xl">
      ...
      {/* Main Viewport */}
      <div className="relative flex flex-col items-center justify-center bg-slate-950 border border-slate-800 rounded-lg p-6 min-h-[180px] overflow-hidden">
        ...
        <div className="absolute bottom-2 right-2 flex items-center gap-1 bg-slate-900/80 backdrop-blur border border-slate-800 rounded px-2 py-1">
          <button
          ...
          {[2, 4, 8].map((z) => (
            <button
              key={z}
              onClick={() => setZoom(z)}
              className={`px-1.5 py-0.5 font-mono text-[10px] rounded ${zoom === z ? 'bg-cyan-600 text-white font-bold' : 'text-slate-400 hover:text-slate-200'}`}
            >
              {z}x
            </button>
          ))}
        ...
        <div className="flex items-center gap-1">
          {(['down', 'up', 'left', 'right'] as const).map((dir) => (
            <button
              ...
              className={`px-2 py-1 text-[10px] font-mono capitalize rounded transition ${direction === dir ? 'bg-cyan-700 text-white font-bold' : 'bg-slate-900 text-slate-400 hover:text-slate-200 border border-slate-800'}`}
            >
              {dir}
            </button>
          ))}
      ...
      <div className="space-y-2 text-[11px] font-mono">
        <div className="flex items-center justify-between text-slate-400">
          <span>Type:</span>
          <span className="text-cyan-400 font-bold">{asset.type}</span>
        </div>
        <div className="flex items-center justify-between text-slate-400">
        ...
        <div className="pt-2 border-t border-slate-800 space-y-1">
          ...
          <div className="flex flex-wrap gap-1">
            {(asset.tags || []).map((t) => (
              <span
                key={t}
                className="px-1.5 py-0.5 bg-slate-950 text-cyan-300 border border-slate-800 rounded text-[10px]"
              >
                #{t}
              </span>
            ))}
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-slate-800">
        <button
          ...
          className="flex-1 py-1.5 bg-slate-950 hover:bg-slate-800 text-slate-300 border border-slate-800 rounded text-[11px] font-mono flex items-center justify-center gap-1 transition"
        >
        ...
        {onSelect && (
          <button
            ...
            className="flex-1 py-1.5 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold rounded text-[11px] font-mono flex items-center justify-center gap-1 shadow-lg transition"
          >
