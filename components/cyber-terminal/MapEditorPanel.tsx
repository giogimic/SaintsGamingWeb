...
export default function MapEditorPanel({ onClose }: { onClose: () => void }) {
  const [activeTab, setActiveTab] = useState<'tiles' | 'collision' | 'rpg_nodes' | 'npcs' | 'encounters' | 'gates'>('tiles');
  const [mapName, setMapName] = useState('tuxemon_route_1');
  const [mapSlug, setMapSlug] = useState('route_1');
  ...
  const [encounterSpecies, setEncounterSpecies] = useState<string>('tuxemon_001');
  ...
  const handleSaveMap = async () => {
    ...
    try {
      ...
      const res = await saveWorldMap({
        id: mapSlug,
        name: mapName,
        gridData: JSON.stringify(dummyGrid),
    ...
    <div className="absolute inset-0 z-50 bg-[#12141a]/95 text-slate-100 flex flex-col font-mono select-none overflow-hidden backdrop-blur-md border-4 border-cyan-600/80 rounded-lg shadow-2xl">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 bg-slate-900/90 border-r border-slate-800 flex flex-col p-3 gap-3 overflow-y-auto">
          {/* Mode Switcher Tabs */}
          <div className="grid grid-cols-3 gap-1 bg-black/40 p-1 rounded border border-slate-800">
            <button
          ...
          {activeTab === 'encounters' && (
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-slate-300">Beast Species</label>
                <input
                  type="text"
              ...
              <div className="flex gap-2">
                <div className="flex-1 flex flex-col gap-1">
                  <label className="text-xs text-slate-300">Min Level</label>
                  <input
                    type="number"
                    min={1}
                ...
                <div className="flex-1 flex flex-col gap-1">
                  <label className="text-xs text-slate-300">Max Level</label>
                  <input
                    type="number"
                    min={1}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-slate-300">Encounter Rate (0.01 - 0.50)</label>
                <input
                  type="number"
                  step="0.05"
                  min={0.01}
